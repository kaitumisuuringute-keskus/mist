    ...
    it("should get full worker info") {
      ...
      when(execution.getWorkerLink(any[String]))
        .thenReturn(Some(WorkerLink(
          "id", "test", None,
          WorkerInitInfo(Map(), 20, Duration.Inf, Duration.Inf, "test", "localhost:0", "localhost:0", 1073741824, ""))))
      ...
      Get("/v2/api/workers/id") ~> route ~> check {
        ...
        resp.initInfo shouldBe WorkerInitInfo(Map(), 20, Duration.Inf, Duration.Inf, "test", "localhost:0","localhost:0", 1073741824, "")
